...
class TextProcessing():
    ...
    def process_entire_text(self, wiki, title):
        ...
        # t4 = threading.Thread(target=self.processInfobox, args=(seperator[0],))
        # t5 = threading.Thread(target=self.processBody, args=(seperator[0],))
        # t4.start()
        # t5.start()
        # if(len(seperator) > 1):
        #     t1 = threading.Thread(
        #     target=self.processExternalLinks, args=(seperator[1],))
        #     t2 = threading.Thread(
        #     target=self.processCategories, args=(seperator[1],))
        #     t3 = threading.Thread(
        #     target=self.processReferences, args=(seperator[1],))
        #     t1.start()
        #     t2.start()
        #     t3.start()
        #     t1.join()
